...
//Speeds
#define SPEED_VOR_OHNE_BALL		245
#define SPEED_ZURUECK			245
#define SPEED_SEITLICH			245

//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//!!!!!!!!!!!!!!!!!!!!!IR ENTFERNUNGSSENSOR HINTEN IS HIN!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
uint8_t Run_main(void)
{
  ...
  */
  ...
  Spez_Feld_Pos=0;		////!!!!!!!!!!!!!!!ABSTANDSSENSOR HINTEN IS HIN!!!!!!!!!!!
  /*
  0=???
  1=??? Links
  2=??? Rechts
  ...
  */
  ...
  
  {
	...
	if(twinkel==10)				//Relativwinkel zum Vergleich speichern(wenn timer-winkel über 10)	
	{
		oldwinkel=RelativWinkel;
		twinkel=0;
	}
	
	if(twinkel==5) oldwinkel2=RelativWinkel;	//2ter Vergleichswert für Relativwinkel
	twinkel++;
   ...
   if((TASTER4()==1))
  {
   Exit=1;
	...
	if (STATUS==1)							
	{
	 ...
	 {
		 if(stat<7)	//Roboter hat ball (?nicht?)
		 {
			 ...
			 {
			  if(BALL_ENTFERNUNG>20)	//Toleranz (wenn drunter wahrscheinlich irgendwo in der Pampa hinten)
			  {
		 ...
		 if(IR_DIRECTION==8) //Ball rechts seitlich
		 {
		  if(BALL_ENTFERNUNG>120)	//Wenn Ball nahe
			{
				stat=1;		//zurück
	...
	/*	if(Spez_Feld_Pos==9&&ENTFERNUNGEN[HINTEN]>185)	//Wenn Ball nicht gefunden und Hinten Links an der Wand							!!!AUSPROBIEREN!!!
	 ...
	 if(stat==1)
	 {
		MotorLinksSpeed =   SPEED_ZURUECK;
		MotorRechtsSpeed = -SPEED_ZURUECK;
		MotorHintenSpeed = 0;
		//zurück
	 }
	 ...
	 if(stat==2)
	 {
		MotorLinksSpeed = -SPEED_VOR_OHNE_BALL;
		MotorRechtsSpeed = SPEED_VOR_OHNE_BALL;
		MotorHintenSpeed = 0;
		//vor
	 }
	 ...
	 {
		MotorLinksSpeed = 0;
		MotorRechtsSpeed = SPEED_SEITLICH;
		MotorHintenSpeed = -SPEED_SEITLICH;
		//vornelinks
	 }
	 
	 if(stat==4)
	 {
		MotorLinksSpeed = -SPEED_SEITLICH;
		MotorRechtsSpeed = 0;
		MotorHintenSpeed = SPEED_SEITLICH;
		//vornerechts
	 }
	 ...
	 if(stat==25)
	 {
		//Rechts fahren== -20 -20 250
		MotorLinksSpeed = -20;
		MotorRechtsSpeed = -20;
		MotorHintenSpeed = 250;
		//Rechts
	 }
	 
	 if(stat==26)
	 {
		//Links fahren== 20 20 -250
		MotorLinksSpeed = 20;
		MotorRechtsSpeed = 20;
		MotorHintenSpeed = -250;
		//Links
	 ...
	 if(stat==7)
	 {
		...
		if(Iforyou>20)	//Wenn Ballverlusttimer > 20 ==> Ball Verloren
		{
			SCHUSS();	//Amal nachballern
	//		stat=1;
	 ...
	 //------------------------------------------------------------------------------------------------------------------------------------
     //Winkelkorrektur
		if(smoothturn!=1&&(stat<8||stat>11))	//Wenn nicht gerade in einer Drehung und smoothturn ungleich 1
		{
			...
			else
			{
				if((RelativWinkel>WINKEL_KORREKTUR*(-1)-30)||(RelativWinkel<WINKEL_KORREKTUR+30))
				{
					...
					if(smoothturn==0)
					{
						KorrekturMotoren=-KorrekturMotoren*3;
						smoothturn=1;
					}
				}
				else
		...
		{
			if(oldwinkel>oldwinkel2-3&&oldwinkel<oldwinkel2+3)
			{
				smoothturn=2;
				KorrekturMotoren=0;
			}
