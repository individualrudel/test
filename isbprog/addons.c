...
uint8_t change=0;
 ...
 if(TASTER1_GEDRUECKT==1) //BUTTON LEFT PRESSED
	{
 ...
 if(TASTER2_GEDRUECKT==1) //BUTTON RIGHT PRESSED
	{
	 if(change==0)
	 {
 ...
 if(TASTER3()==1) //BUTTON ACCCEPT PRESSED
  ...
  if(State==3)	//Fahren
  {
   ...
   sprintf(Zahl,"%3d",Speedh);
   for (uint8_t i=0; i<=2; i++) LINE_2[i+10]=Zahl[i];
  }
...
void SUB2_1(void)
{
 ...
 TASTER2();
  ...
  {
   if(Topspeed>=5) Topspeed=Topspeed-5; //BUTTON LEFT PRESSED
  }
  
  if(TASTER2_GEDRUECKT==1)
  {
   if(Topspeed<=250) Topspeed=Topspeed+5; //BUTTON RIGHT PRESSED
  }
  ...
  strcpy(LINE_1,"Topspeed");
  
  sprintf(Zahl,"%3d",Topspeed);
  for (uint8_t i=0; i<=2; i++) LINE_2[i+0]=Zahl[i];
  ...
  if(TASTER4()==1) //BUTTON BACK PRESSED
	...
	if(TASTER3()==1) //BUTTON ACCEPT PRESSED
	{
	 exit=1;
	}
 }
 
 if(exit==2)Topspeed=oldtopspeed;
 exit=0;
}
